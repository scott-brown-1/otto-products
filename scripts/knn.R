...
prepped_recipe <- setup_train_recipe(train, encode=F, poly=F, smote_K=0, pca_threshold=0.85) 
#TODO: ENCODE! 
...
## Predict new y
output <- predict(final_wf, new_data=test, type='prob') %>%2
  bind_cols(test$id,.) %>%
  rename(
    id=...1,
...
vroom::vroom_write(output,'./outputs/knn_preds.csv',delim=',')
